...
Необходимые лаги модели искались из коэффициентов частичной автокорреляционной функции (PACF). Значения, выходящие за границы ±2/√n (где n — длина ряда), считаются статистически значимыми и используются для построения модели.
...
Для модели с наилучшими метриками построено распределение ошибки. Оно аппроксимировалось нормальным распределением:
...
## 🔎 Результаты и выводы
**Оптимальный лаг:** Автоматический подбор по информационным критериям (AIC, BIC, HQC) показал, что оптимальное число лагов составляет около N (вставь своё значение). Это подтверждается значимыми коэффициентами PACF.

**Распределение ошибок:** Остатки модели аппроксимируются нормальным распределением с небольшими отклонениями в хвостах, что подтверждается Q-Q графиком и статистическими тестами (например, тест Льюнга-Бокса).

**Прогноз:** Модель успешно прогнозирует производство электроэнергии на 20 месяцев вперёд с доверительным интервалом 95%. Интервалы достаточно узкие, что говорит о высокой уверенности модели в прогнозах.
...
**Ограничения:** Несмотря на хорошее соответствие модели данным, небольшие отклонения от нормальности в хвостах остатков могут указывать на редкие, но сильные выбросы. Кроме того, модель AR ограничена в способности учитывать сезонные эффекты и другие сложные паттерны во временных рядах.
...
# Перед запуском можно изменить параметры в config.yaml (например, путь к данным, максимальный лаг и др.)
...
```bash
python src/main.py \
  --input_data_path src/Electric_Production.csv \
  --log_diff True \
  --max_lag 30 \
...
| Аргумент            | Описание                                            | Тип     | Пример                |
| ------------------- | --------------------------------------------------- | ------- | --------------------- |
| `--config`          | Путь к YAML конфигурационному файлу                 | `str`   | `configs/config.yaml` |
| `--input_data_path` | Путь к CSV с исходными данными                      | `str`   | `src/data.csv`        |
| `--log_diff`        | Применять логарифмическую разность (`True`/`False`) | `bool`  | `True`                |
| `--model_type`      | Тип модели (на данный момент только `AR`)           | `str`   | `AR`                  |
| `--max_lag`         | Максимальное число лагов для перебора               | `int`   | `50`                  |
| `--distribution`    | Распределение ошибок (`gaussian` или `student`)     | `str`   | `gaussian`            |
| `--forecast_steps`  | Количество шагов прогноза вперед                    | `int`   | `20`                  |
| `--confidence`      | Уровень доверительного интервала                    | `float` | `0.95`                |
| `--images_dir`      | Путь для сохранения графиков                        | `str`   | `images/`             |


## ⚙️ Конфигурация (config.yaml)

- `input_data_path`: путь к CSV с данными.
- `preprocessing.log_diff`: применять ли логарифмическую разность для стабилизации ряда.
- `model.model_type`: тип модели (AR).
- `model.max_lag`: максимальное число лагов для перебора.
- `model.distribution`: распределение ошибок ('gaussian', 'student').
- `forecast.forecast_steps`: количество месяцев (или шагов) для прогноза.
- `forecast.confidence`: уровень доверительного интервала.
- `images_dir`: путь к папке для сохранения графиков.
